...
let attrs =
  choose
    [
      const Xmldiff.Nmap.empty; map [ elem_name; ident ] Xmldiff.Nmap.singleton;
    ]
  ...
  lazy
    (choose
       [
         ...
         map [ ident ] (fun s -> `D s);
         map
           [ elem_name; attrs; list (unlazy xml) ]
           (fun s attrs elems ->
             let rec normalise = function
               | ([] | [ _ ]) as x -> x
               | (`E _ as el) :: xs -> el :: normalise xs
               | `D s :: xs -> (
...
let test_diff xml1 xml2 =
  let _patch, xml3 = Xmldiff.diff_with_final_tree xml1 xml2 in
  check_eq ~pp:pp_xml xml2 xml3

let suite = ("xmldiff", [ test_case "diff" [ xml; xml ] test_diff ])
