...
(** pretty-printers for items generated by Alcobar; useful for the user in
...
(** [unlazy gen] forces the generator [gen]. It is useful when defining
    ...
    let rec a_gen =
      lazy
        (choose
           [
             map [ int ] (fun i -> A i); map [ unlazy a_gen ] (fun s -> Self s);
           ])
...
(** [dynamic_bind gen f] is a monadic bind, it allows to express the generation
    ...
    [examples/serializer/] directory of the Alcobar implementation.

    Because the structure of a generator built with [dynamic_bind] is
    opaque/dynamic (it depends on generated values), the Alcobar library cannot
...
(** [test_case name generators test_fn] creates a test case: [test_fn] is run
    with random data produced by [generators]. Any failure is reported annotated
    with [name]. *)
...
(** [failf format ...] generates a test failure and prints the message specified
    by the format string [format] and the following arguments. It is set up so
    that [%a] calls for an ['a printer] and an ['a] value. *)
...
(** [check_eq pp cmp eq x y] evaluates whether x and y are equal, and if they
    are not, raises a failure and prints an error message. Equality is evaluated
    as follows:
    ...
    If [pp] is provided, use this to print [x] and [y] if they are not equal. If
    [pp] is not provided, a best-effort printer will be generated from the
