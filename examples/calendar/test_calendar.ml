...
let time =
  map [ int64 ] (fun a ->
      try C.from_mjd (Int64.to_float a /. 100_000_000_000_000.)
      with CalendarLib.Date.Out_of_bounds -> bad_test ())

let pp_time ppf t =
  pp ppf "%04d-%02d-%02d %02d:%02d:%02d" (C.year t)
...
let test_add_sub t1 t2 =
  guard (C.compare t1 t2 < 0);
  check_eq ~pp:pp_time ~eq:C.equal (C.add t1 (C.precise_sub t2 t1)) t2

let test_add_period t p =
  let t' = C.add t p in
  let t'' = C.rem t' p in
  check_eq ~pp:pp_time ~eq:C.equal t t''

let suite =
  ( "calendar",
    [
      test_case "add_sub" [ time; time ] test_add_sub;
      test_case "add_period" [ time; period ] test_add_period;
    ] )
