...
let check_map ((list, map) : t) =
  ...
  in
  let list =
    match List.stable_sort (fun a b -> compare (fst a) (fst b)) list with
    | [] -> []
    | (k, v) :: rest -> (k, v) :: dedup k rest
  in
      ...
      choose
        [
          ...
          map [ uint8; uint8; map_gen ] (fun k v (l, m) ->
              ((k, v) :: l, Map.add k v m));
          map [ uint8; uint8 ] (fun k v -> ([ (k, v) ], Map.singleton k v));
          map [ uint8; map_gen ] (fun k (l, m) ->
              let rec rem_all k l =
                let l' = List.remove_assoc k l in
                if l = l' then l else rem_all k l'
              in
...
let test_map m = check (check_map m)
let suite = ("map", [ test_case "map" [ map_gen ] test_map ])
